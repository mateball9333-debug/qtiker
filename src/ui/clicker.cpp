...
#include "ui/casino_dialog.h"
#include "ui/carat_dialog.h"
#include "ui/gacha_dialog.h"
#include "core/game_rules.h"
#include "ui/particle_overlay.h"
#include "ui/release_notes.h"
#include "ui/settings_dialog.h"
#include "ui/statistics_dialog.h"
...
void Clicker::showGacha() {
    ...
    connect(dialog, &GachaDialog::upgradeRequested, this, [this, dialog](int index) {
        upgradeCard(index);
        dialog->setArchCount(game.arches);
        dialog->setInventory(game.cardCounts, game.cardUpgradeLevel, game.cardUpgradePath, game.selectedCard, game.selectedCard2);
        refreshUi();
    });
    connect(dialog, &GachaDialog::upgradePathRequested, this, [this, dialog](int index, int path) {
        upgradeCardPath(index, path);
        dialog->setArchCount(game.arches);
        dialog->setInventory(game.cardCounts, game.cardUpgradeLevel, game.cardUpgradePath, game.selectedCard, game.selectedCard2);
        refreshUi();
    });
...
void Clicker::buildUi() {
    ...
    casinoButton->setIconSize(TopIconSize);
...
void Clicker::applyThemeIcons() {
    ...
    setThemeIcon(casinoButton, ":/assets/ui/casino.svg");
}
...
void Clicker::refreshUi() {
    ...
    const int hoardDisplayBonus = totalSpecialEffectValue(GachaEffect::Hoarder);
    if (hoardDisplayBonus > 0 && game.score > 0) {
        displayedIncome = applyMultiplier(displayedIncome,
            100 + hoardDisplayBonus * qMin(game.score / 10000, Q_INT64_C(400)), 100);
    }
    if (isPathActive(GachaEffect::Hoarder, 2) && game.arches > 0) {
        const int pct = debugGachaCardAt(9).specialBase
            ...
            * HoardPathB_PerArch;
        displayedIncome = applyMultiplier(displayedIncome, 100 + pct * game.arches, 100);
    }
    ...
    if (isPathActive(GachaEffect::ArchHopper, 2)) {
        ...
        if (archBonus > 0 && game.arches > 0)
            displayedIncome = applyMultiplier(displayedIncome, 100 + archBonus * game.arches / 3, 100);
    }

    displayedIncome <<= game.incomeMultLevel;

    auto displayedClick = applyActiveCardBonus(game.perClick, GachaEffect::Click);
    displayedClick = applyTimedBuffBonuses(displayedClick, TimedBuffEffect::Click);
    const int archDisplayBonus = totalSpecialEffectValue(GachaEffect::ArchHopper);
    if (archDisplayBonus > 0 && game.arches > 0)
        displayedClick = applyMultiplier(displayedClick, 100 + archDisplayBonus * game.arches, 100);
    if (isPathActive(GachaEffect::Speedrun, 2))
        displayedClick *= 2;
    displayedClick <<= game.clickMultLevel;
...
QString Clicker::formatUpgradeText(const QString &label, int ownedCount, qint64 cost) const {
